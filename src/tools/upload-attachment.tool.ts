...
const toolHandler = async (args: any) => {
  ...
  try {
    ...
    if (response.isError) {
      ...
      logger.error(`Failed to upload attachment: ${response.error}`, undefined, { entityType: args.entity_type, entityId: args.entity_id });
      return {
    ...
    logger.info("Attachment uploaded successfully", { entityType: args.entity_type, entityId: args.entity_id });
    return {
      content: [
    ...
    logToolResponse(toolName, false, Date.now() - startTime);
    logger.error("Failed to upload attachment", error, { entityType: args?.entity_type, entityId: args?.entity_id });
    throw error;
  }
};
