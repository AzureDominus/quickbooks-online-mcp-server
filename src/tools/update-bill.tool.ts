...
const BillLineSchema = z.object({
  ...
  AccountBasedExpenseLineDetail: z.object({
    ...
    }).optional().describe("Tax code reference"),
  }).optional().describe("Account-based expense details"),
  ...
  }).optional().describe("Item-based expense details"),
});
...
const UpdateBillInputSchema = z.object({
  ...
  }).optional().describe("Vendor reference"),
  Line: z.array(BillLineSchema).optional().describe("Bill line items"),
  TxnDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional().describe("Bill date (YYYY-MM-DD)"),
  DueDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional().describe("Payment due date (YYYY-MM-DD)"),
...
const toolHandler = async (args: { [x: string]: any }) => {
  ...
  try {
    ...
    return {
      content: [
        { type: "text" as const, text: "Bill updated successfully:" },
