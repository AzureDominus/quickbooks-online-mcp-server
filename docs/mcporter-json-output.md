# MCP tool output normalization (mcporter)
...
- `mcporter call QuickBooks.<tool> --output json` (direct tool invocation)
- `bin/quickbooks-mcp` wrapper for mcporter server command (auto-builds before running)

This workflow works best when a tool returns **a single JSON payload**.
